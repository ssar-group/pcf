...
impl<'a> Parser<'a> {
    ...
    pub fn parse_program(&mut self) -> ParseResult {
        if self.tokens.is_empty() {
            return ParseResult { program: Some(Program::default()), diagnostics: Vec::new() };
        }
        ...
        while !self.is_at_end() {
            match self.current_kind() {
                Some(TokenKind::Output) => {
                    if let Some(item) = self.parse_output_statement() { items.push(item); }
                }
                Some(TokenKind::Eof) | None => break,
                Some(_) => {
                    if let Some(token) = self.current() { self.diagnostics.push(unexpected_token(token)); }
                    self.advance();
                }
            }
            self.skip_separators();
        ...
        ParseResult { program: Some(Program { items, span }), diagnostics: std::mem::take(&mut self.diagnostics) }
    }

    fn parse_output_statement(&mut self) -> Option<Item> {
        let output_token = self.advance().map(Clone::clone)?;
        let string_token = self.current().map(Clone::clone)?;

        match string_token.kind {
            TokenKind::String(value) => {
                self.advance();
                let expression = LiteralExpression { value: Literal::String(value), span: string_token.span };
                Some(Item::Statement(Statement::Output(OutputStatement {
                    value: expression,
                    span: Span { start: output_token.span.start, end: string_token.span.end },
                })))
            }
            _ => {
                self.diagnostics.push(expected_output_string(&output_token, &string_token));
    ...
    fn synchronize_output_statement(&mut self) {
        while !self.is_at_end() {
            match self.current_kind() {
                Some(TokenKind::Newline | TokenKind::Semicolon | TokenKind::RightBrace | TokenKind::Eof) | None => break,
                Some(_) => { self.advance(); }
            }
        }
    }
    ...
    fn current_kind(&self) -> Option<&TokenKind> { self.current().map(|token| &token.kind) }
    ...
    fn skip_separators(&mut self) {
        while matches!(self.current_kind(), Some(TokenKind::Newline | TokenKind::Semicolon)) { self.advance(); }
    }
}

pub fn parse(tokens: &[Token]) -> ParseResult { Parser::new(tokens).parse_program() }

fn program_span(items: &[Item]) -> Option<Span> {
    let first = items.first()?;
    let last = items.last()?;
    Some(Span { start: item_span(first).start, end: item_span(last).end })
}
...
fn unexpected_token(token: &Token) -> Diagnostic {
    Diagnostic {
        ...
        labels: vec![Label { span: token.span, message: Some("this token is not valid here".to_string()), primary: true }],
        notes: Vec::new(),
    }
}

fn expected_output_string(output_token: &Token, token: &Token) -> Diagnostic {
    let span = if matches!(token.kind, TokenKind::Eof) { output_token.span } else { token.span };
    Diagnostic {
        severity: Severity::Error,
        code: DiagnosticCode("PCF1001"),
        message: "expected string literal after `output`".to_string(),
        labels: vec![Label { span, message: Some("only string literals are supported".to_string()), primary: true }],
...
mod tests {
    ...
    fn parses_output_statements_in_order() {
        ...
        let values: Vec<_> = program
            ...
            .collect();
        assert_eq!(values, vec![Literal::String("hello".to_string()), Literal::String("world".to_string())]);
    }
