...
fn inspect(args: InspectArgs) -> Result<ExitStatus> {
    ...
    report.section("Output", |section| {
        if tokens_selected { render_tokens(section, &lex_result.tokens); }
        if ast_selected {
            if tokens_selected { section.blank(); }
            section.section("AST", |ast| match parse_result.program {
                Some(program) if parse_result.diagnostics.is_empty() => render_text_lines(ast, format!("{program:#?}")),
                Some(program) => {
                    render_text_lines(ast, format!("{program:#?}"));
                    ast.line("parse completed with diagnostics");
                }
                None => ast.line("parse failed: no program produced"),
            });
        }
    });
    report.blank();
...
fn parse_command(args: FileArgs) -> Result<ExitStatus> {
    ...
    report.section("Diagnostics", |section| render_diagnostic_summary(section, &diagnostics));
    report.blank();
    report.section("Output", |section| match parse_result.program {
        Some(program) => section.section("AST", |ast| render_text_lines(ast, format!("{program:#?}"))),
    ...
    let exit_status = if has_errors(&diagnostics) { ExitStatus::Failure } else { ExitStatus::Success };
    print_output(report.finish(footer_status(exit_status), started.elapsed()));
    Ok(exit_status)
}
...
fn check_command(args: FileArgs) -> Result<ExitStatus> {
    ...
    let exit_status = if has_errors(&check_result.diagnostics) { ExitStatus::Failure } else { ExitStatus::Success };
    print_output(report.finish(footer_status(exit_status), started.elapsed()));
    Ok(exit_status)
}
...
fn render_diagnostic_output(section: &mut ui::SectionBuilder, diagnostics: &[Diagnostic], file: &Path, source: &str) {
    for (index, diagnostic) in diagnostics.iter().enumerate() {
        section.section(ui::format_diagnostic_title(diagnostic), |entry| ui::render_diagnostic_labels(entry, diagnostic, file, source));
        if index + 1 != diagnostics.len() { section.blank(); }
    }
}

fn render_check_content(section: &mut ui::SectionBuilder, outputs: &[pcf::CheckOutput]) {
    section.section("Output", |output| {
        if outputs.is_empty() { output.line("No output"); return; }
        for item in outputs { output.line(&item.content); }
    });
}

fn render_tokens<T: std::fmt::Debug>(section: &mut ui::SectionBuilder, tokens: &[T]) {
    section.section("Tokens", |tokens_section| {
        for token in tokens { tokens_section.line(format!("{token:?}")); }
    });
}

fn render_text_lines(section: &mut ui::SectionBuilder, text: String) {
    for line in text.lines() { section.line(line); }
}
...
fn footer_status(status: ExitStatus) -> FooterStatus {
    match status { ExitStatus::Success => FooterStatus::Completed, ExitStatus::Failure => FooterStatus::Failed }
}
